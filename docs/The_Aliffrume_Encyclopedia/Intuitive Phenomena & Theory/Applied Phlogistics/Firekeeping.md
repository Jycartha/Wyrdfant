---
aliases:
  - firekeeping
---
# Firekeeping
The study and practice of applying [phlogistics](../../Glossary.md#Phlogistics) to influence material reality, including engineering, medicine, and many other fields.

## Usage
All firekeeping begins with the generation of [Soulfire](../Pure%20Phlogistics/Soul/Soulfire.md), which is then manipulated in some manner to achieve some effect. It can be suddenly compressed to exert an explosive pressure, run over [Flamescript](../Soulfire/Flamescript.md) to affect [engrams](../Pure%20Phlogistics/Engram.md), or used for its [fluid mechanics](../Pure%20Phlogistics/Phlogistic%20Model.md#Fluidic%20Pressure), in increasing order of complexity.

All life can control ambient [Soulfire](../Pure%20Phlogistics/Soul/Soulfire.md) and even produce its own, but sourcing it from [Gimyndine](../Soulfire/Gimyndine/Gimyndine.md) is much more potent and allows easier use when integrated with [Kindlegem](../../Technology/Kindlegem.md). This led to [Gimyndine](../Soulfire/Gimyndine/Gimyndine.md) technology and its derivatives becoming the forefront of development for centuries, and many self-sourced firekeeping techniques became relegated to folk contexts or small communities, where the usage of [Gimyndine](../Soulfire/Gimyndine/Gimyndine.md) was either impractical from extenuating circumstance, or rejected on principle.

## Fields of Firekeeping
### Artifice
### Arterial Arts
### Coupled Engineering
### Phlogistic Computing
### Phlogistic Engineering
### Ichorcraft
### Synthesis
### Soulsmithing
### Oversense